...
public:
    ...
    /*for(auto &i:list_of_purchase)
       {
          //print orders
          json j1;
          cout<<i.order.size()<<endl;
          for(auto &orders:i.order)
          {
                j1["item_code"]=orders.item_code;
                j1["quantity"]=orders.quantity;
                j1["price"]=orders.price;
          }
            json j2;
            j2["pid"]=i.pid;
            j2["name_of_customer"]=i.name_of_customer;
            j2["date"]=i.date;
      ...
      /*  for(auto x:list_of_purchase)
        {
            json j1;
            j1["pid"]=x.pid;
            j1["name_of_customer"]=x.name_of_customer;
            j1["date"]=x.date;
            for(auto x:x.order)
            {
                json j2;
                j2["item_code"]=x.item_code;
                j2["quantity"]=x.quantity;
                j2["price"]=x.price;
                j1["order"].push_back(j2);
            }
            j.push_back(j1);
